...
# Расстояние между соседними звёздами (D).
# Подобрано так, чтобы внешние орбиты смежных звёзд пересекались ровно на 1 орбиту:
# D чуть больше R_max_odd + R_max_even - ORBIT_DR, чтобы предпоследние не касались:
# 9 + 6 = 15 > 14.5 (последние пересекаются)
# 8 + 6 = 14 < 14.5 (предпоследние НЕ пересекаются)
STAR_SPACING = 14.5
...
# Нечётные: 8 орбит, max = 2 + 7*1 = 9
# Чётные:   5 орбит,  max = 2 + 4*1 = 6
